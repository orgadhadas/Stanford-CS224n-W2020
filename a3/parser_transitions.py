...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "LA":
            new_dependency = (self.stack[-1], self.stack[-2])
            self.dependencies.append(new_dependency)
            self.stack.pop(-2)
        if transition == "RA":
            new_dependency = (self.stack[-2], self.stack[-1])
            self.dependencies.append(new_dependency)
            self.stack.pop(-1)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_parses = [p for p in partial_parses if len(p.buffer) > 0]

    while len(unfinished_parses) > 0:
        n_batches = len(unfinished_parses) // batch_size + 1
        for i in range(n_batches):
            batch = unfinished_parses[i*batch_size:(i+1)*batch_size]
            transitions = model.predict(batch)
            for partial_parse, trans in zip(batch,transitions):
                partial_parse.parse_step(trans)
